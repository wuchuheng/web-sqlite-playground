  ...
  T.g("Basic sanity checks")
    .t({
      ...
      test: function (sqlite3) {
        for (const k of ["SQLITE_CONFIG_GETMALLOC", "SQLITE_CONFIG_URI"]) {
          T.assert(capi[k] > 0);
        }
        T.assert(
          capi.SQLITE_MISUSE === capi.sqlite3_config(capi.SQLITE_CONFIG_URI, 1),
        ...
        if (0) {
          ...
          T.assert(0 === capi.sqlite3_config(capi.SQLITE_CONFIG_URI, 1));
        }
      },
    }) /*sqlite3_config()*/
    ...
    .t("Namespace object checks", function (sqlite3) {
      ...
      T.assert(wasmCtypes.structs[0].name === "sqlite3_vfs")
        ...
        .assert(
          wasmCtypes.structs[1 /*sqlite3_io_methods*/].members.xFileSize
            .offset > 0
        );
      [
      ...
      T.assert(
        capi.sqlite3_errstr(capi.SQLITE_IOERR_ACCESS).indexOf("I/O") >= 0
      )
        .assert(
          capi.sqlite3_errstr(capi.SQLITE_CORRUPT).indexOf("malformed") > 0
        )
      ...
      function (sqlite3) {
        ...
        // alloc(), realloc(), allocFromTypedArray()
        {
          ...
          for (let i = 0; i < u.length; ++i) {
            T.assert(u[i] === byteList[i]).assert(u[i] === w.peek8(m + i));
          }
          w.dealloc(m);
...

        {
          ...
          T.assert(17 === w.poke8(m, 17).peek8(m))
            ...
            .assert(T.eqApprox(345678.9, w.poke32f(m, 345678.9).peek32f(m)))
            .assert(
        ...
        //log("jstrcpy()...");
        {
          ...
          T.assert(1 === rc, "Must not write partial multi-byte char.").assert(
            0 === ua[0]
          );
          refill();
        ...
        //log("xWrap()...");
        {
          ...
          if (haveWasmCTests()) {
            if (!sqlite3.config.useStdAlloc) {
              fw = w.xWrap("sqlite3__wasm_test_str_hello", "utf8:dealloc", [
                "i32",
              ]);
              rc = fw(0);
              T.assert("hello" === rc);
    ...
    .t(
      {
        ...
        test: function (sqlite3) {
          ...
          T.mustThrowMatching(() => K(), /via 'new'/).mustThrowMatching(
            () => new K("hi"),
            /^Invalid pointer/
          );
          const k1 = new K(),
          ...
          try {
            ...
            T.assert("number" === typeof k1.pointer).mustThrowMatching(
              () => (k1.pointer = 1),
              /pointer/
            );
            k1.$p4 = 1;
          ...
          try {
            ...
            T.assert(1 === counter)
              ...
              .assert(
                wts.memberToJsString("cstr") === wts.memberToJsString("$cstr")
              )
              .mustThrowMatching(
  ...
  T.g("sqlite3_randomness()")
    .t("To memory buffer", function (sqlite3) {
      ...
      try {
        ...
        T.assert(0 === wasm.peek8(p)).assert(0 === wasm.peek8(p + n - 1));
  ...
  T.g("sqlite3.oo1")
    .t({
      ...
      test: function (sqlite3) {
        ...
        T.assert(wasm.isPtr(db.pointer))
          ...
          .assert(
            0 === sqlite3.capi.sqlite3_extended_result_codes(db.pointer, 1)
          )
        ...
        T.assert(ex instanceof sqlite3.SQLite3Error)
          ...
          .assert(
            0 === ex.message.indexOf("SQLITE_MISUSE: sqlite3 result code")
          )
    ...
    .t("sqlite3_db_config() and sqlite3_db_status()", function (sqlite3) {
      ...
      try {
        ...
        rc = capi.sqlite3_status(capi.SQLITE_STATUS_MEMORY_USED, pCur, pHi, 0);
    ...
    .t("DB.Stmt", function (sqlite3) {
      ...
      try {
        T.assert(wasm.isPtr(st.pointer))
          ...
          .assert(
            capi.sqlite3_stmt_status(st, capi.SQLITE_STMTSTATUS_RUN, 0) === 0
          )
          .assert(!st._mayGet)
      ...
      T.mustThrowMatching(
        ...
        function (err) {
          return (
            ...
            0 <
              err.message.indexOf("Do not call the Stmt constructor directly.")
          );
        }
      );
    })
    ...
    .t(
        ...
        if (1) {
          ...
          wasm.scopedAllocCall(() => {
            ...
            for (const v of vfsList) {
              ...
              T.assert(wasm.isPtr(pVfs)).assert(pVfs === vfsArg(v));
      ...
      */
        ...
        T.assert(
          pVfsDflt === duv(db.pointer, 0) || pVfsMem === duv(db.pointer, 0)
    ...
    .t("Table t", function (sqlite3) {
      ...
      db.exec(new TextEncoder("utf-8").encode("SELECT a a, b b FROM t"), {
        ...
        callback: function (row, stmt) {
          ...
          T.assert(
            ...
            .assert(this.columnNames[0] === "a" && this.columnNames[1] === "b")
      ...
      db.exec("SELECT a a, b b FROM t", {
        ...
        callback: function (row, stmt) {
          ...
          T.assert(Array.isArray(row)).assert(
            (0 === row[1] % 2 && row[1] < 7) || row[1] instanceof Uint8Array
          );
        },
      });
      T.assert(8 === counter);
      ...
      try {
        T.assert(0 === st.columnCount).assert(false === st.isReadOnly());
        const ndx = st.getParamIndex(":b");
        T.assert(1 === ndx);
        st.bindAsBlob(ndx, "ima blob")
          /*step() skipped intentionally*/ .reset(true);
      } finally {
        T.assert(0 === st.finalize()).assert(undefined === st.finalize());
      }
    ...
    .t("sqlite3_table_column_metadata()", function (sqlite3) {
      const stack = wasm.pstack.pointer;
      try {
        const [pzDT, pzColl, pNotNull, pPK, pAuto] = wasm.pstack.allocPtr(5);
    ...
    .t({
      ...
      test: function (sqlite3) {
        ...
        T.assert(blobRc instanceof Uint8Array).assert(2 === blobRc.length);
        //debug("blobRc=",blobRc);
        ...
        T.assert(sqlite3.capi.SQLITE_MISUSE === rc, "For invalid arg count.");
    ...
    .t(
      {
        ...
        test: function (sqlite3) {
          ...
          T.assert(
            18 === db.selectValue("select summerN(1,8,9), summerN(2,3,4)")
          );
          T.mustThrowMatching(() => {
    ...
    .t("ATTACH", function (sqlite3) {
      ...
      T.assert(
        2 === db.selectValue("select a from foo.bar where a>1 order by a")
      );
      ...
      const pCb = wasm.installFunction(
        ...
        function (pVoid, nCols, aVals, aCols) {
          ...
          T.assert(2 === nCols)
            ...
            .assert(
              +wasm.cstrToJs(wasm.peekPtr(aVals + wasm.ptrSizeof)) ===
      ...
      try {
        ...
        rc = capi.sqlite3_exec(
          db,
          new TextEncoder("utf-8").encode("select a, a*2 from foo.bar"),
          pCb,
          0,
          0
        );
          ...
          try {
            ...
            if (w.bigIntEnabled) {
              ...
              T.assert(
                BigInt(2 * o64) ===
                  biTimes2(
                    BigInt(o64) /*explicit conv. required to avoid TypeError
                                           in the call :/ */
                  )
              );
              ...
              T.assert(minMaxI64[0] < BigInt(Number.MIN_SAFE_INTEGER)).assert(
              ...
              T.assert(g64(pMin) === minMaxI64[0], "int64 mismatch").assert(
              ...
              T.mustThrowMatching(() => {
                db.prepare("select ?").bind(minMaxI64[0] - BigInt(1));
              }, rxRange).mustThrowMatching(
                () => {
                  db.prepare("select ?").bind(minMaxI64[1] + BigInt(1));
                },
    ...
    .t({
      ...
      test: function (sqlite3) {
        ...
        */
          ...
          methods: {
            ...
            xClose: function (pCursor) {
              try {
                ...
                T.assert(c instanceof capi.sqlite3_vtab_cursor).assert(
                  !VT.xCursor.get(pCursor)
                );
                c.dispose();
                return 0;
              } catch (e) {
              ...
              try {
                ...
                if (pii.$nConstraint > 0) {
                  ...
                  for (let i = 0; i < max; ++i) {
                    ...
                    T.assert(v instanceof sii.sqlite3_index_constraint).assert(
                      v.pointer >= pii.$aConstraint
                    );
                    v.dispose();
                    ...
                    T.assert(
                      v instanceof sii.sqlite3_index_constraint_usage
                ...
                if (pii.$nOrderBy > 0) {
                  ...
                  for (let i = 0; i < max; ++i) {
                    ...
                    T.assert(wasm.isPtr(v));
                    v = pii.nthOrderBy(i);
                    T.assert(v instanceof sii.sqlite3_index_orderby).assert(
                      v.pointer >= pii.$aOrderBy
                    );
                    v.dispose();
                  }
                }
        ...
        let rc = capi.sqlite3_create_module(this.db, "testvtab", tmplMod, 0);
        this.db.checkRc(rc);
    ...
    .t(
      {
        ...
        predicate: () =>
          isUIThread() || "local/sessionStorage are unavailable in a Worker",
        test: function (sqlite3) {
        ...
        let rc = capi.sqlite3_preupdate_hook(
          ...
          function (p, pDb, op, zDb, zTbl, iKey1, iKey2) {
            ...
            switch (op) {
              ...
              case capi.SQLITE_UPDATE:
                T.assert(
                  "number" === typeof capi.sqlite3_preupdate_new_js(pDb, 0)
                );
                break;
              case capi.SQLITE_DELETE:
                T.assert(
                  "number" === typeof capi.sqlite3_preupdate_old_js(pDb, 0)
                );
                break;
              default:
                toss("Unexpected hook operator:", op);
    ...
    test: function (sqlite3) {
      ...
      try {
        ...
        T.assert("bTwo" === db1.selectValue("select b from t where rowid=2"))
          .assert(
            undefined === db1.selectValue("select a from t where rowid=1")
          )
        ...
        if (testSessionEnable) {
          ...
          T.assert(rc > 0)
            ...
            .assert(
              undefined === db1.selectValue("select a from t where rowid=2")
            );
        } else {
        ...
        T.assert("b2" === db1.selectValue("select b from t where rowid=2"))
          ...
          .assert(
            undefined === db1.selectValue("select b from t where rowid=4")
          );
        ...
        T.assert(0 === rc)
          ...
          .assert(
            undefined === db2.selectValue("select b from t where rowid=1")
          );
        if (testSessionEnable) {
          T.assert(
            undefined === db2.selectValue("select b from t where rowid=2"),
          ...
          T.assert(
            "bTwo" === db2.selectValue("select b from t where rowid=2")
  ...
  T.g(
  ...
  ).t(
    {
      ...
      test: async function (sqlite3) {
        ...
        T.assert(db instanceof sqlite3.oo1.DB).assert(1 === u1.getFileCount());
        db.exec([
        ...
        if (1) {
          ...
          T.assert(2 == u1.getFileCount()).assert(dbytes.byteLength == nWrote);
    ...
    .t({
      ...
      test: async function (sqlite3) {
        ...
        const tryOne = function (vfsName, descr) {
          const uri = "file:///foo_create_by_me.db";
          const args = uri + (vfsName ? "?vfs=" + vfsName : "");
