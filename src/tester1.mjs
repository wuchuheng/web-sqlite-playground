...

    {
        ...
        if (isUIThread()) {
            ...
            logClass = function (cssClass, ...args) {
                ...
                if (cssClass) {
                    for (const c of Array.isArray(cssClass)
                        ? cssClass
                        : [cssClass]) {
                        ln.classList.add(c);
                    }
                ...
                ln.append(
                    document.createTextNode(normalizeArgs(args).join(" "))
                );
                logTarget.append(ln);
            };
            ...
            const cbReverseIt = () => {
                logTarget.classList[cbReverse.checked ? "add" : "remove"](
                    "reverse"
                );
  ...
  */
    ...
    */
        assert: function f(expr, ...msg) {
            ...
            if (!this.toBool(expr)) {
                throw new Error(
                    msg.length ? msg.join(" ") : "Assertion failed."
                );
            }
            return this;
        },
    ...
    */
        mustThrowMatching: function (f, filter, msg) {
            ...
            if (!pass) {
                throw new Error(
                    msg || "Filter rejected this exception: " + err.message
                );
            }
            return this;
        },
        ...
        throwUnless: function (expr, msg) {
            ++this.counter;
            if (!this.toBool(expr))
                throw new Error(msg || "throwUnless() failed");
            return this;
        },
        eqApprox: (v1, v2, factor = 0.05) =>
            v1 >= v2 - factor && v1 <= v2 + factor,
        TestGroup: (function () {
            let groupCounter = 0;
    ...
    T.g("Basic sanity checks")
        .t({
            ...
            test: function (sqlite3) {
                ...
                T.assert(
                    capi.SQLITE_MISUSE ===
                        capi.sqlite3_config(capi.SQLITE_CONFIG_URI, 1),
                ...
                if (0) {
                    log(
                        "We cannot _fully_ test sqlite3_config() after the library",
                    ...
                    T.assert(
                        0 === capi.sqlite3_config(capi.SQLITE_CONFIG_URI, 1)
                    );
                }
            },
        }) /*sqlite3_config()*/
        ...
        .t({
            ...
            test: function (sqlite3) {
                if (sqlite3.config.useStdAlloc) {
                    warn(
                        "Using system allocator. This violates the docs and",
                    ...
                    T.assert(wasm.alloc.impl === wasm.exports.sqlite3_malloc)
                        ...
                        .assert(
                            wasm.realloc.impl === wasm.exports.sqlite3_realloc
                        );
                }
            },
        })
        .t("Namespace object checks", function (sqlite3) {
            ...
            T.assert(wasmCtypes.structs[0].name === "sqlite3_vfs")
                ...
                .assert(
                    wasmCtypes.structs[1 /*sqlite3_io_methods*/].members
                        .xFileSize.offset > 0
                );
            [
            ...
            [
                /* Spot-check a few of the WASM API methods. */ "alloc",
                "dealloc",
                "installFunction",
            ].forEach((k) => T.assert(wasm[k] instanceof Function));

            T.assert(
                capi.sqlite3_errstr(capi.SQLITE_IOERR_ACCESS).indexOf("I/O") >=
                    0
            )
                .assert(
                    capi
                        .sqlite3_errstr(capi.SQLITE_CORRUPT)
                        .indexOf("malformed") > 0
                )
            ...
            try {
                ...
                T.assert(3 === e.cause).assert("test" === e.message);
            }
            try {
                throw new sqlite3.WasmAllocError("test", "ing", ".");
            } catch (e) {
                T.assert("test ing ." === e.message);
            }

            try {
                throw new sqlite3.SQLite3Error(capi.SQLITE_SCHEMA);
            } catch (e) {
                T.assert("SQLITE_SCHEMA" === e.message).assert(
                    capi.SQLITE_SCHEMA === e.resultCode
                );
            }
            try {
                sqlite3.SQLite3Error.toss(capi.SQLITE_CORRUPT, { cause: true });
            } catch (e) {
            ...
            try {
                sqlite3.SQLite3Error.toss("resultCode check");
            } catch (e) {
                T.assert(capi.SQLITE_ERROR === e.resultCode).assert(
                    "resultCode check" === e.message
                );
            }
        })
        ////////////////////////////////////////////////////////////////////
            ...
            function (sqlite3) {
                ...
                //log("heap getters...");
                {
                    ...
                    for (const n of li) {
                        ...
                        T.assert(bpe === s.BYTES_PER_ELEMENT).assert(
                            w.heapForSize(s.constructor) === s
                        );
                ...
                // alloc(), realloc(), allocFromTypedArray()
                {
                    ...
                    T.assert(
                        "number" ===
                            typeof sqlite3.capi.SQLITE_MAX_ALLOCATION_SIZE
                    );
                    if (!sqlite3.config.useStdAlloc) {
                        const tooMuch =
                                sqlite3.capi.SQLITE_MAX_ALLOCATION_SIZE + 1,
                            isAllocErr = (e) =>
                    ...
                    for (let i = 0; i < u.length; ++i) {
                        T.assert(u[i] === byteList[i]).assert(
                            u[i] === w.peek8(m + i)
                        );
                    }
                    w.dealloc(m);
...

                {
                    ...
                    T.assert(17 === w.poke8(m, 17).peek8(m))
                        ...
                        .assert(
                            T.eqApprox(
                                345678.9,
                                w.poke32f(m, 345678.9).peek32f(m)
                            )
                        )
                        .assert(
                            T.eqApprox(
                                4567890123.4,
                                w.poke64f(m, 4567890123.4).peek64f(m)
                            )
                        );
                    if (w.bigIntEnabled) {
                        T.assert(
                ...
                // isPtr32()
                {
                    ...
                    T.assert(ip(0))
                        ...
                        .assert(!ip())
                        .assert(
                            !ip(null) /*might change: under consideration*/
                        );
                }

                //log("jstrlen()...");
                {
                    T.assert(3 === w.jstrlen("abc")).assert(
                        4 === w.jstrlen("äbc")
                    );
                }
                ...
                //log("jstrcpy()...");
                {
                    ...
                    T.assert(2 === rc)
                        .assert(fillChar !== ua[1])
                        .assert(fillChar === ua[2]);
                } /*jstrcpy()*/
                ...
                //log("scopedAlloc() and friends...");
                {
                    ...
                    try {
                        ...
                        T.assert("number" === typeof z1)
                            .assert(z2 > z1)
                            .assert(z3 > z2)
                            .assert(
                                0 === w.peek32(z1),
                                "allocPtr() must zero the targets"
                            )
                    ...
                    w.scopedAllocCall(function () {
                        T.assert(1 === w.scopedAlloc.level);
                        const [cstr, n] = w.scopedAllocCString(
                            "hello, world",
                            true
                        );
                        T.assert(12 === n)
                ...
                //log("xCall()...");
                {
                    ...
                    T.assert(Number.isFinite(pJson)).assert(
                        w.cstrlen(pJson) > 300
                    );
                }
                ...
                {
                    T.mustThrowMatching(
                        () => w.xWrap("sqlite3_libversion", null, "i32"),
                        /requires 0 arg/
                    )
                        .assert(
                            w.xWrap.resultAdapter("i32") instanceof Function
                        )
                    ...
                    if (haveWasmCTests()) {
                        ...
                        if (w.bigIntEnabled) {
                            ...
                            w.scopedAllocCall(function () {
                                ...
                                T.assert(!Number.isSafeInteger(r1)).assert(
        ...
        .t(
            {
                ...
                predicate: (sqlite3) =>
                    !!sqlite3.wasm.exports.sqlite3__wasm_test_struct ||
                ...
                test: function (sqlite3) {
                    ...
                    try {
                        T.assert(k1.constructor === K)
                            ...
                            .assert(
                                K.prototype.lookupMember("p4").key === "$p4"
                            )
                            .assert(
                                K.prototype.lookupMember("$p4").name === "p4"
                            )
                            .mustThrowMatching(
                                () => K.prototype.lookupMember("nope"),
                                /not a mapped/
                            )
                            .assert(
                                undefined ===
                                    K.prototype.lookupMember("nope", false)
                            )
                            .assert(k1 instanceof StructType)
                            .assert(StructType.isA(k1))
                            .mustThrowMatching(() => (k1.$ro = 1), /read-only/);
                        Object.keys(MyStructDef.members).forEach(function (
                            key
                        ) {
                            key = K.memberKey(key);
                            T.assert(
                        ...
                        if (MyStructDef.members.$p8) {
                            ...
                            T.assert(
                                BigInt(2 * Number.MAX_SAFE_INTEGER) === k1.$p8
                            );
                        }
                        ...
                        T.assert(Array.isArray(k1.ondispose))
                            ...
                            .assert(
                                "A C-string." === k1.memberToJsString("cstr")
                            );
                        k1.$pP = k2;
                        ...
                        let ptr = k1.pointer;
                        k1.dispose();
                        T.assert(undefined === k1.pointer).mustThrowMatching(
                            () => {
                                k1.$pP = 1;
                            },
                            /disposed instance/
                    ...
                    if (!W.bigIntEnabled) {
                        log(
                            "Skipping WasmTestStruct tests: BigInt not enabled."
                        );
                        return;
                    }

                    const WTStructDesc = W.ctype.structs.filter(
                        (e) => "WasmTestStruct" === e.name
                    )[0];
                    ...
                    try {
                        T.assert(wts.constructor === WTStruct)
                            ...
                            .assert(wts.pointer > 0)
                            .assert(0 === wts.$v4)
                            .assert(0n === wts.$v8)
                            .assert(0 === wts.$ppV)
                            .assert(0 === wts.$xFunc);
                        const testFunc = W.xGet(
                            "sqlite3__wasm_test_struct" /*name gets mangled in -O3 builds!*/
                        );
                        let counter = 0;
              ...
              "which itself was called from JS. arg =",arg);*/
                        ...
                        wts.$v4 = 10;
                        wts.$v8 = 20;
                        wts.$xFunc = W.installFunction(
                            wtsFunc,
                            wts.memberSignature("xFunc")
                        );
                        T.assert(0 === counter)
                            .assert(10 === wts.$v4)
                            .assert(20n === wts.$v8)
                            .assert(0 === wts.$ppV)
                            .assert("number" === typeof wts.$xFunc)
                            .assert(0 === wts.$cstr)
                            .assert(wts.memberIsString("$cstr"))
                            .assert(!wts.memberIsString("$v4"))
                            .assert(null === wts.memberToJsString("$cstr"))
                            .assert(
                                W.functionEntry(wts.$xFunc) instanceof Function
                            );
             ...
             testing the struct-wrapping API. */
                        ...
                        T.assert(1 === counter)
                            ...
                            .assert(
                                "string" === typeof wts.memberToJsString("cstr")
                            )
                            .assert(
                                wts.memberToJsString("cstr") ===
                                    wts.memberToJsString("$cstr")
                            )
                            .mustThrowMatching(
                                () => wts.memberToJsString("xFunc"),
              ...
              that exception propagates back here... */
                        ...
                        T.assert(Array.isArray(wts.ondispose))
                            ...
                            .assert(
                                "A C-string." === wts.memberToJsString("cstr")
                            );
                    ...
                    if (1) {
                        // ondispose of other struct instances
                        const s1 = new WTStruct(),
                            s2 = new WTStruct(),
                            s3 = new WTStruct();
                        T.assert(s1.lookupMember instanceof Function).assert(
                            s1.addOnDispose instanceof Function
                        );
                        ...
                        T.assert(
                            !s2.pointer,
                            "Expecting s2 to be ondispose'd by s1."
                        );
                        T.assert(
                            !s3.pointer,
                            "Expecting s3 to be ondispose'd by s2."
                        );
                    }
                },
            } /*StructBinder*/
        ...
        .t(
            ...
            function (sqlite3) {
                ...
                try {
                    ...
                    let p1 = P.alloc(12);
                    T.assert(p1 === stack - 16 /*8-byte aligned*/).assert(
                        P.pointer === p1
                    );
                    let p2 = P.alloc(7);
                    T.assert(
                        p2 === p1 - 8 /*8-byte aligned, stack grows downwards*/
                    )
                    ...
                    let p3 = P.alloc(n);
                    T.assert(p3 === stack - remaining).mustThrowMatching(
                        () => P.alloc(1),
                        isAllocErr
                    );
                } finally {
                ...
                try {
                    ...
                    T.assert(
                        P.pointer ===
                            stack - 16 /*always rounded to multiple of 8*/
                    )
                        .assert(p2 === p1 + 4)
                        .assert(p3 === p2 + 4);
                    T.mustThrowMatching(
                        () => P.allocChunks(1024, 1024 * 16),
                        (e) => e instanceof sqlite3.WasmAllocError
                    );
                } finally {
                    P.restore(stack);
    ...
    T.g("sqlite3_randomness()")
        .t("To memory buffer", function (sqlite3) {
            ...
            try {
                ...
                T.assert(0 === wasm.peek8(p)).assert(
                    0 === wasm.peek8(p + n - 1)
                );
        ...
        .t("To byte array", function (sqlite3) {
            ...
            T.assert(0 === n).assert(ta === capi.sqlite3_randomness(ta));
    ...
    T.g("sqlite3.oo1")
        .t({
            ...
            test: function (sqlite3) {
                    ...
                    before: function (db) {
                        while (this.disposeBefore.length) {
                            ...
                            if (wasm.isPtr(v)) wasm.dealloc(v);
                                ...
                                v instanceof sqlite3.StructBinder.StructType
                            ) {
                                v.dispose();
                ...
                T.assert(wasm.isPtr(db.pointer))
                    ...
                    .assert(
                        0 ===
                            sqlite3.capi.sqlite3_extended_result_codes(
                                db.pointer,
                                1
                            )
                    )
                    ...
                    .assert(
                        db.pointer === wasm.xWrap.testConvertArg("sqlite3*", db)
                    );
                ...
                T.assert(ex instanceof sqlite3.SQLite3Error)
                    ...
                    .assert(
                        0 ===
                            ex.message.indexOf(
                                "SQLITE_MISUSE: sqlite3 result code"
                            )
                    )
        ...
        .t("sqlite3_db_config() and sqlite3_db_status()", function (sqlite3) {
            ...
            T.assert(0 === rc);
            rc = capi.sqlite3_db_config(
                this.db,
                capi.SQLITE_DBCONFIG_MAX + 1,
                0
            );
            T.assert(capi.SQLITE_MISUSE === rc);
            rc = capi.sqlite3_db_config(
        ...
        .t("DB.Stmt", function (sqlite3) {
            ...
            T.mustThrowMatching(
                () => new sqlite3.oo1.Stmt("hi"),
                function (err) {
                    return (
                        err instanceof sqlite3.SQLite3Error &&
                        capi.SQLITE_MISUSE === err.resultCode &&
                        0 <
                            err.message.indexOf(
                                "Do not call the Stmt constructor directly."
                            )
                    );
                }
            );
      ...
      */
        ...
        .t("Table t", function (sqlite3) {
            ...
            T.assert(2 === vals.length).assert(1 === vals[0] && 3 === vals[1]);
            vals = db.selectValues(
                "select a from t order by a limit $L",
                { $L: 2 },
                capi.SQLITE_TEXT
            );
            T.assert(2 === vals.length).assert(
                "1" === vals[0] && "3" === vals[1]
            );
            vals = undefined;

            let blob = db.selectValue("select b from t where a='blob'");
            T.assert(blob instanceof Uint8Array).assert(
                0x68 === blob[0] && 0x69 === blob[1]
            );
            blob = null;
            let counter = 0,
                colNames = [];
            ...
            db.exec(new TextEncoder("utf-8").encode("SELECT a a, b b FROM t"), {
                ...
                callback: function (row, stmt) {
                    ...
                    T.assert(
                        ...
                        .assert(this.columnNames === colNames)
                        .assert(
                            this.columnNames[0] === "a" &&
                                this.columnNames[1] === "b"
                        )
                        .assert((row.a % 2 && row.a < 6) || "blob" === row.a);
                },
            });
            ...
            T.assert(2 === colNames.length).assert(
                "a" === colNames[0] && "B" === colNames[1]
            );
            list.length = 0;
            db.exec("SELECT a a, b b FROM t", {
                ...
                callback: function (row, stmt) {
                    ++counter;
                    T.assert(Array.isArray(row)).assert(
                        (0 === row[1] % 2 && row[1] < 7) ||
          ...
          had been passed. */
           ...
           https://github.com/emscripten-core/emscripten/issues/17391 */
                T.mustThrow(() =>
                    db.selectValue("SELECT " + (Number.MAX_SAFE_INTEGER + 1))
                ).mustThrow(() =>
                    db.selectValue("SELECT " + (Number.MIN_SAFE_INTEGER - 1))
                );
            }

            let st = db.prepare("update t set b=:b where a='blob'");
            try {
                T.assert(0 === st.columnCount).assert(
                    false === st.isReadOnly()
                );
                ...
                T.assert(0 === st.finalize()).assert(
                    undefined === st.finalize()
                );
            }
            ...
            try {
                ...
                T.assert(e instanceof sqlite3.SQLite3Error).assert(
                    0 == e.message.indexOf("Cannot prepare empty")
                );
            }
        ...
        .t({
            ...
            predicate: () =>
                !!wasm.exports.sqlite3_set_authorizer ||
                "Missing sqlite3_set_authorizer()",
            test: function (sqlite3) {
                T.assert(capi.SQLITE_IGNORE > 0).assert(capi.SQLITE_DENY > 0);
                const db = this.db;
        ...
        */
                rc = ssa(db, null, 0);
                authCount = 0;
                T.assert(-9999 != db.selectValue("select a from t")).assert(
                    0 === authCount
                );
                rc = ssa(
                    db,
                ...
                T.assert(0 === rc);
                T.assert(n === db.selectValue("select count(*) from t")).assert(
                    authCount > 0
                );
                authCount = 0;
                rc = ssa(
        ...
        ////////////////////////////////////////////////////////////////////////
        .t(
            "selectArray/Object/Values() via INSERT/UPDATE...RETURNING",
            function (sqlite3) {
                let rc = this.db.selectObject(
                    "INSERT INTO t(a,b) VALUES(83,84) RETURNING a as AA"
                );
                T.assert(83 === rc.AA);
                rc = this.db.selectArray(
                    "UPDATE T set a=85 WHERE a=83 RETURNING b as BB"
                );
        ...
        .t({
            ...
            test: function (sqlite3) {
                ...
                db.createFunction("bar", {
                    ...
                    xFunc: (pCx, ...args) => {
                        T.assert(
                            db.pointer === capi.sqlite3_context_db_handle(pCx)
                        );
                        let rc = 0;
                ...
                T.assert(0 === db.selectValue("select bar()"))
                    ...
                    .assert(
                        T.eqApprox(1.3, db.selectValue("select asis(1 + 0.3)"))
                    );
                ...
                T.assert(blobRc instanceof Uint8Array).assert(
                    2 === blobRc.length
                );
                //debug("blobRc=",blobRc);
           ...
           multiple arities... */
                ...
                T.mustThrowMatching(
                    () => db.selectValue(sqlFArity0),
                    (e) =>
                        e instanceof sqlite3.SQLite3Error &&
        ...
        .t(
            {
              ...
              sqlite3_aggregate_context() properly. */
                    ...
                    T.assert(15 === v);
                    T.mustThrowMatching(
                        () => db.selectValue("select summer(1,2)"),
                    ...
                    T.assert(
                        18 ===
                            db.selectValue(
                                "select summerN(1,8,9), summerN(2,3,4)"
                            )
                    );
                    T.mustThrowMatching(() => {
        ...
        .t(
            {
                name: "Window UDFs",
                predicate: (sqlite3) =>
                    !!sqlite3.wasm.exports.sqlite3_create_window_function ||
           ...
           https://sqlite.org/windowfunctions.html#udfwinfunc */
                    const db = this.db;
                    const sjac = (cx, n = 4) =>
                        capi.sqlite3_js_aggregate_context(cx, n);
                    ...
                    T.assert(Array.isArray(rc)).assert(5 === rc.length);
                    let count = 0;
                    for (const row of rc) {
                        switch (++count) {
                            case 1:
                                T.assert("a" === row[0] && 9 === row[1]);
                                break;
                            case 2:
                                T.assert("b" === row[0] && 12 === row[1]);
                                break;
                            case 3:
                                T.assert("c" === row[0] && 16 === row[1]);
                                break;
                            case 4:
                                T.assert("d" === row[0] && 12 === row[1]);
                                break;
                            case 5:
                                T.assert("e" === row[0] && 9 === row[1]);
                                break;
                            default:
                                toss("Too many rows to window function.");
                        }
                    }
                    const resultRows = [];
                    rc = db.exec({
                    ...
                    T.assert(rc === resultRows).assert(5 === rc.length);
        ...
        .t("ATTACH", function (sqlite3) {
            ...
            T.assert(3 === resultRows.length).assert(2 === resultRows[1]);
            T.assert(
                2 ===
                    db.selectValue("select a from foo.bar where a>1 order by a")
            );
            ...
            let rc = capi.sqlite3_exec(
                db,
                "select a, a*2 from foo.bar",
                function (aVals, aNames) {
         ...
         signature, as opposed to the simplified JS-specific one... */
            ...
            const pCb = wasm.installFunction(
                "i(pipp)",
                function (pVoid, nCols, aVals, aCols) {
           ...
           aVals); */
                    ...
                    T.assert(2 === nCols)
                        ...
                        .assert(
                            +wasm.cstrToJs(
                                wasm.peekPtr(aVals + wasm.ptrSizeof)
                            ) ===
            ...
            try {
                ...
                db2.exec("attach 'attached.db' as foo");
                T.mustThrow(
                    () => db2.exec("insert into foo.t(a) values(2)"),
                ...
                test: function () {
                    ...
                    try {
                        ...
                        if (w.bigIntEnabled) {
                            ...
                            T.assert(
                                BigInt(2 * o64) ===
                                    biTimes2(
                                        BigInt(
                                            o64
                                        ) /*explicit conv. required to avoid TypeError
                                           in the call :/ */
                                    )
                            );
                            ...
                            T.assert(
                                minMaxI64[0] < BigInt(Number.MIN_SAFE_INTEGER)
                            ).assert(
                                minMaxI64[1] > BigInt(Number.MAX_SAFE_INTEGER)
                            );
                            ...
                            T.assert(
                                g64(pMin) === minMaxI64[0],
                                "int64 mismatch"
                            ).assert(
                                g64(pMax) === minMaxI64[1],
                                "int64 mismatch"
                            );
                            ...
                            T.assert(g64(pMin) === minMaxI64[0])
                                .assert(
                                    minMaxI64[0] ===
                                        db.selectValue("select ?", g64(pMin))
                                )
                                .assert(
                                    minMaxI64[1] ===
                                        db.selectValue("select ?", g64(pMax))
                                );
                            const rxRange = /too big/;
                            T.mustThrowMatching(() => {
                                db.prepare("select ?").bind(
                                    minMaxI64[0] - BigInt(1)
                                );
                            }, rxRange).mustThrowMatching(
                                () => {
                                    db.prepare("select ?").bind(
                                        minMaxI64[1] + BigInt(1)
                                    );
                                },
        ...
        .t({
            ...
            predicate: (sqlite3) =>
                !!sqlite3.capi.sqlite3_vtab || "Missing vtab support",
            test: function (sqlite3) {
        ...
        */
                    ...
                    methods: {
                        ...
                        xOpen: function (pVtab, ppCursor) {
                            try {
                                ...
                                T.assert(t instanceof capi.sqlite3_vtab).assert(
                                    c instanceof capi.sqlite3_vtab_cursor
                                );
                                c._rowId = 0;
                                return 0;
                            } catch (e) {
                        ...
                        xFilter: function (
                            ...
                            try {
                                const c = VT.xCursor.get(pCursor);
                                c._rowId = 0;
                                const list = capi.sqlite3_values_to_js(
                                    argc,
                                    argv
                                );
                            ...
                            try {
                                ...
                                if (pii.$nConstraint > 0) {
                                    ...
                                    for (let i = 0; i < max; ++i) {
                                        ...
                                        T.assert(
                                            v instanceof
                                                sii.sqlite3_index_constraint_usage
                                        ).assert(
                                            v.pointer >= pii.$aConstraintUsage
                                        );
                ...
                T.assert(0 === tmplMod.$xUpdate)
                    ...
                    .assert(
                        tmplMod.$xCreate === tmplMod.$xConnect,
        ...
        */
        ...
        .t("Custom collation", function (sqlite3) {
            ...
            let myCmp = function (pArg, n1, p1, n2, p2) {
                ...
                ++collationCounter;
                const rc = wasm.exports.sqlite3_strnicmp(
                    p1,
                    p2,
                    n1 < n2 ? n1 : n2
                );
            ...
            let rc = capi.sqlite3_create_collation_v2(
                this.db,
                "mycollation",
                capi.SQLITE_UTF8,
                0,
            ...
            rc = capi.sqlite3_create_collation(
                this.db,
                "hi",
                capi.SQLITE_UTF8 /*not enough args*/
            );
            T.assert(capi.SQLITE_MISUSE === rc);
            rc = capi.sqlite3_create_collation_v2(
                this.db,
                "hi",
                capi.SQLITE_UTF8 + 1 /*invalid encoding*/,
                0,
                0,
                0
            );
            T.assert(capi.SQLITE_FORMAT === rc).mustThrowMatching(
                () => this.db.checkRc(rc),
        ...
        .t(
            {
                ...
                test: function (sqlite3) {
                    ...
                    const unlink = (this.kvvfsUnlink = () =>
                        JDb.clearStorage(this.kvvfsDbFile));
                    unlink();
                    ...
                    try {
                        ...
                        T.assert(
                            3 === db.selectValue("select count(*) from kvvfs")
                        );
                        db.close();
                        db = new JDb(filename);
                        db.exec("insert into kvvfs(a) values(4),(5),(6)");
                        T.assert(
                            6 === db.selectValue("select count(*) from kvvfs")
                        );
                    } finally {
                        db.close();
                    }
    ...
    T.g("Hook APIs")
        .t({
            ...
            test: function (sqlite3) {
                ...
                let rc = capi.sqlite3_commit_hook(
                    db,
                    (p) => {
                        ++countCommit;
                ...
                T.assert(1 === countCommit, "But EXCLUSIVE transactions do.");
                db.transaction((d) => {
                    T.assert(0 == capi.sqlite3_get_autocommit(db));
                ...
                T.mustThrowMatching(
                    ...
                    (e) => {
                        return (
                            capi.SQLITE_MISUSE === e.resultCode &&
                    ...
                    (p, op, dbName, tbl, rowid) => {
                        ...
                        switch (op) {
                            ...
                            case capi.SQLITE_DELETE:
                                ...
                                break;
                            default:
                                toss("Unexpected hook operator:", op);
                        }
                    },
                    3
                );
        ...
        .t({
            ...
            predicate: () =>
                capi.sqlite3_preupdate_hook || "Missing pre-update hook API",
            test: function (sqlite3) {
                const db = new sqlite3.oo1.DB(":memory:", 1 ? "c" : "ct");
                const countHook = Object.create(null);
                let rc = capi.sqlite3_preupdate_hook(
                    db,
                    function (p, pDb, op, zDb, zTbl, iKey1, iKey2) {
                        T.assert(9 === p)
                        ...
                        switch (op) {
                            ...
                            case capi.SQLITE_UPDATE:
                                T.assert(
                                    "number" ===
                                        typeof capi.sqlite3_preupdate_new_js(
                                            pDb,
                                            0
                                        )
                                );
                            ...
                            case capi.SQLITE_DELETE:
                                T.assert(
                                    "number" ===
                                        typeof capi.sqlite3_preupdate_old_js(
                                            pDb,
                                            0
                                        )
                                );
        ...
        }) /*pre-update hooks*/; /*end hook API tests*/

    ////////////////////////////////////////////////////////////////////////
    T.g("Auto-extension API").t({
        name: "Auto-extension sanity checks.",
        test: function (sqlite3) {
            let counter = 0;
            const fp = wasm.installFunction(
                "i(ppp)",
                function (pDb, pzErr, pApi) {
                    ++counter;
                    return 0;
                }
            );
    ...
    T.g("Session API").t({
        ...
        test: function (sqlite3) {
            ...
            db1.exec(sqlInit);
            db2.exec(sqlInit);
            T.assert(3 === db1.selectValue("select count(*) from t")).assert(
                "b3" === db1.selectValue("select b from t where rowid=3")
            );
            ...
            try {
                ...
                T.assert(
                    "bTwo" === db1.selectValue("select b from t where rowid=2")
                )
                    .assert(
                        undefined ===
                            db1.selectValue("select a from t where rowid=1")
                    )
                    .assert(
                        "b4" ===
                            db1.selectValue("select b from t where rowid=4")
                    )
                    .assert(3 === db1.selectValue("select count(*) from t"));

                const testSessionEnable = false; /* it's not yet clear whether these test failures are
                         broken tests or broken bindings. */
                if (testSessionEnable) {
                    rc = capi.sqlite3session_enable(pSession, 0);
                    T.assert(0 === rc).assert(
                        0 === capi.sqlite3session_enable(pSession, -1)
                    );
                    ...
                    T.assert(rc > 0)
                        ...
                        .assert(
                            undefined ===
                                db1.selectValue("select a from t where rowid=2")
                        );
                } else {
                ...
                rc = capi.sqlite3session_changeset(
                    pSession,
                    pnChanges,
                    ppChanges
                );
             ...
             and nChanges for application to db2. */
                rc = capi.sqlite3changeset_invert(
                    nChanges,
                    pChanges,
                    pnChanges,
                    ppChanges
                ...
                rc = capi.sqlite3changeset_apply(
                    ...
                    wasm.peekPtr(ppChanges),
                    0,
                    (pCtx, eConflict, pIter) => {
                        return 1;
                    },
                    0
                );
                ...
                T.assert(
                    "b2" === db1.selectValue("select b from t where rowid=2")
                )
                    .assert(
                        "a1" ===
                            db1.selectValue("select a from t where rowid=1")
             ...
             db2... */
                ...
                T.assert(0 === rc)
                    .assert(
                        "b4" ===
                            db2.selectValue("select b from t where rowid=4")
                    )
                    .assert(
                        "aThree" ===
                            db2.selectValue("select a from t where rowid=3")
                    )
                    .assert(
                        undefined ===
                            db2.selectValue("select b from t where rowid=1")
                    );
                if (testSessionEnable) {
                    T.assert(
                        undefined ===
                            db2.selectValue("select b from t where rowid=2"),
                        "But... the session was disabled when rowid=2 was deleted?"
                    );
                    log(
                        "rowids from db2.t:",
                        db2.selectValues("select rowid from t order by rowid")
                    );
    ...
    }) /*session API sanity tests*/; /*end of session API group*/
    ...
    T.g(
        ...
        .t(
            {
                ...
                test: async function (sqlite3) {
                    ...
                    const filename = (this.opfsDbFile =
                        "/dir/sqlite3-tester1.db");
                    const fileUri =
                        "file://" + filename + "?delete-before-open=1";
                    const initSql = [
                        "create table p(a);",
                    ...
                    try {
                        db.exec(initSql);
                        T.assert(
                            3 === db.selectValue("select count(*) from p")
                        );
                        db.close();
                        db = new sqlite3.oo1.OpfsDb(filename);
                        db.exec("insert into p(a) values(4),(5),(6)");
                        T.assert(
                            6 === db.selectValue("select count(*) from p")
                        );
                        this.opfsDbExport = capi.sqlite3_js_db_export(db);
                        T.assert(
                            this.opfsDbExport instanceof Uint8Array
                        ).assert(
                    ...
                    try {
                        ...
                        db.exec(
                            initSql
                        ) /* will throw if delete-before-open did not work */;
                        T.assert(
                            3 === db.selectValue("select count(*) from p")
                        );
                    } finally {
                ...
                test: async function (sqlite3) {
                    ...
                    try {
                        ...
                        const reader = async function () {
                            ...
                            const rv = exp.subarray(
                                cursor,
                                cursor + blockSize > end
                                    ? end
                                    : cursor + blockSize
                            );
                            cursor += blockSize;
        ...
        ***************************************************************/
                    ...
                    try {
                        T.assert(await opfs.entryExists(filename));
                        const [dirHandle, filenamePart] =
                            await opfs.getDirForFilename(filename, false);
    ...
    T.g(
    ...
    ).t(
        {
            ...
            test: async function (sqlite3) {
                T.assert(
                    !sqlite3.capi.sqlite3_vfs_find(sahPoolConfig.name)
                ).assert(
                    sqlite3.capi
                        .sqlite3_js_vfs_list()
                ...
                T.assert(u1 === u2)
                    ...
                    .assert(
                        u1.getCapacity() >= sahPoolConfig.initialCapacity
          ...
          */ "create table t(a);",
                ...
                T.assert(1 === u1.getFileCount()).mustThrowMatching(
                    () => u1.pauseVfs(),
                    (err) => {
                        return (
                            capi.SQLITE_MISUSE === err.resultCode &&
                            /^SQLITE_MISUSE: Cannot pause VFS /.test(
                                err.message
                            )
                        );
                    },
                ...
                if (1) {
                    ...
                    const blockSize = 1024,
                        end = dbytes.byteLength;
                    const reader = async function () {
                        if (cursor >= dbytes.byteLength) {
                            return undefined;
                        }
                        const rv = dbytes.subarray(
                            cursor,
                            cursor + blockSize > end ? end : cursor + blockSize
                        );
                        cursor += blockSize;
                        return rv;
                    };
              ...
              following bits will remove them. */
                ...
                let cErr, u3;
                conf2.$testThrowPhase2 = new Error(
                    "Testing throwing during init."
                );
    ...
    T.g("Misc. APIs")
        ...
        .t("Misc. stmt_...", function (sqlite3) {
            const db = new sqlite3.oo1.DB();
            db.exec(
                "create table t(a doggiebiscuits); insert into t(a) values(123)"
            );
            ...
            while (capi.SQLITE_ROW === capi.sqlite3_step(stmt)) {
                ++n;
                T.assert(
                    0 !== capi.sqlite3_stmt_explain(stmt, 1),
                ...
                T.assert(123 === capi.sqlite3_value_int(sv))
                    .assert(
                        "doggiebiscuits" ===
                            capi.sqlite3_column_decltype(stmt, 0)
                    )
    ...
    T.g("Bug Reports")
        .t({
            ...
            predicate: () =>
                wasm.compileOptionUsed("ENABLE_FTS5") || "Missing FTS5",
            test: function (sqlite3) {
          ...
          select rowid from f where path = :path
                ...
                rc = fetchEm();
                T.assert(2 === rc.length).assert("abcghi" === rc.join(""));
                //log('rc =',rc);
                db.close();
            },
        })
        .t({
           ...
           compares that behavior with other VFSes. */
                const tryOne = function (vfsName, descr) {
                    const uri = "file:///foo2.db";
                    let db = new sqlite3.oo1.DB(
                        uri + (vfsName ? "?vfs=" + vfsName : "")
                    );
                    db.exec([
                    ...
                    db = new sqlite3.oo1.DB(
                        uri + "?mode=ro" + (vfsName ? "&vfs=" + vfsName : "")
                    );
                    let err;
                    try {
                    ...
                    T.assert(
                        err && err.message.indexOf("SQLITE_READONLY") === 0
                    );
                    try {
                ...
                let poolUtil;
                await sqlite3
                    .installOpfsSAHPoolVfs(poolConfig)
                    .then((p) => (poolUtil = p));
                T.assert(
                    !!sqlite3.capi.sqlite3_vfs_find(poolConfig.name),
                    "Expecting to find just-registered VFS"
                );
                try {
    ...
    globalThis.sqlite3InitModule.__isUnderTest = true /* disables certain API-internal cleanup so that we can
      ...
      .catch(e=>{
        log("Cannot install OpfsSAHPool.",e);
            ...
            if (haveWasmCTests()) {
                ...
                logClass(
                    "warning",
                    "sqlite3__wasm_test_...() APIs unavailable."
                );
            }
